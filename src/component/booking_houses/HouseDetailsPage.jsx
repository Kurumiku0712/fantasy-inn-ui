...
const HouseDetailsPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setHouseDetails(response.house);
  ...
  const handleConfirmBooking = async () => {
    ...
    const housePricePerNight = houseDetails.housePrice;
    const totalPrice = housePricePerNight * totalDays;
  ...
  const acceptBooking = async () => {
    try {
      ...
      // Make booking
      const response = await ApiService.bookHouse(houseId, userId, booking);
      if (response.statusCode === 200) {
        ...
        setTimeout(() => {
          ...
          navigate('/houses'); // Navigate to houses
        }, 10000);
      }
    } catch (error) {
  ...
  if (isLoading) {
    return <p className='house-detail-loading'>Loading house details...</p>;
  }

  if (error) {
    return <p className='house-detail-loading'>{error}</p>;
  }

  if (!houseDetails) {
    return <p className='house-detail-loading'>House not found.</p>;
  }

  const { houseType, housePrice, housePhotoUrl, description, bookings } = houseDetails;

  return (
    <div className="house-details-booking">
      <h2>House Details</h2>
      <br />
      <img src={housePhotoUrl} alt={houseType} className="house-details-image" />
      <div className="house-details-info">
        <h3>{houseType}</h3>
        <p>Price: ${housePrice} / night</p>
        <p>{description}</p>
      </div>
      ...
      {showMessage && (
        <p className="booking-success-message">
          Booking successful! Confirmation code: {confirmationCode}. An SMS and email of your booking details have been sent to you. Redirecting to All Houses...
        </p>
      )}
      {errorMessage && (
